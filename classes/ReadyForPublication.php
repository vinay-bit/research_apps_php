...
class ReadyForPublication {
    ...
    public function createFromProject($project_id) {
        try {
            ...
            // Check if project already exists in ready for publication
            $check_query = "SELECT id FROM " . $this->table_name . " WHERE project_id = :project_id";
    ...
    public function update($id, $data) {
        ...
        $stmt->bindParam(':first_draft_link', $data['first_draft_link'] ?? null);
        $stmt->bindParam(':plagiarism_report_link', $data['plagiarism_report_link'] ?? null);
    ...
    public function createManual($data) {
        try {
            ...
            // Create ready for publication entry
            $insert_query = "INSERT INTO " . $this->table_name . " 
                            (project_id, paper_title, mentor_affiliation, first_draft_link, plagiarism_report_link, status, notes) 
                            VALUES (:project_id, :paper_title, :mentor_affiliation, :first_draft_link, :plagiarism_report_link, :status, :notes)";
            $insert_stmt = $this->conn->prepare($insert_query);
            ...
            $insert_stmt->bindParam(':first_draft_link', $data['first_draft_link']);
            $insert_stmt->bindParam(':plagiarism_report_link', $data['plagiarism_report_link']);
    ...
    public function getAllProjects() {
        $query = "SELECT p.*, ps.status_name,
                  ...
                  LEFT JOIN project_statuses ps ON p.status_id = ps.id
                  LEFT JOIN users mentor ON p.lead_mentor_id = mentor.id
                  WHERE p.id NOT IN (SELECT project_id FROM " . $this->table_name . ")
    ...
    public function getReadyProjects() {
        $query = "SELECT p.*, ps.status_name,
                  ...
                  AND p.id NOT IN (SELECT project_id FROM " . $this->table_name . ")
                  ORDER BY p.updated_at DESC";
